...
void script_request(lua_State *L, char **buf, size_t *len, const char* request_id, size_t request_id_len) {
    int pop = 1;
...
void script_response(lua_State *L, int status, buffer *headers, buffer *body, const char *request_id, size_t request_id_len) {
...
size_t script_verify_request(lua_State *L) {
    ...
    script_request(L, &request, &len, "1", 1);
